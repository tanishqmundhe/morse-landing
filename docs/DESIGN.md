...
- **The difference, as three panels.** Two problems flat on the sunken ground with a hollow ✕, the answer lifted onto a raised card with a filled ✓ and sage plus-bullets — after the device Verseo use, where the elevation argues before anyone reads a word. It replaced a list of the seven categories and the tools in each: that list was an inventory — true, checkable, and no argument at all.
- **The orbit is the page's one picture, and the artwork is under it rather than in a band of its own.** This page went wrong the first time by putting decoration between a reader and a number, so what artwork it carries has to be doing a job; here it is the ground the orbit turns on. Two rings at 80s and 110s in opposite directions, each mark counter-turning at its ring's rate so it stays upright — a logo rotating with its ring reads as a fairground ride rather than a system. **Morse does not turn.** Everything goes round it, which is the whole claim.
  **`fill="currentColor"` goes on the `<svg>`, not just a text colour.** Some marks carry it inside their own markup and some are bare paths that inherit it; without it on the element the bare ones fell back to black while the others took the ink — half the orbit dark and half light on the same dark ground.
  **A wordmark is sized by width there.** Zoom and Cal.com publish their lettering 24 wide and about 5 tall, so a 22px *height* drew Zoom 97px across, nearly twice the 56px node it sits in. Width is the dimension that has to fit.
  Marks take `ink` rather than `ink-soft` on the orbit: on `float` in `.on-stage` that is 12.3:1 against about 3:1, and a mark that has to be recognised cannot sit at the floor.
