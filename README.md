...
- `public/product/today.jpg`: the app's today card, cropped from a real capture.
- `src/components/logo.tsx`, `unified-machines-logo.tsx`, `src/app/icon.svg`: copied unchanged from Neural-Arc/morse `feat/neural-cal`.
- `docs/DESIGN.md`: the design system of record. Update it with every visual change.

The booking, notes, teleprompter and follow-up demos are illustrative. They create no meetings and send no data.
